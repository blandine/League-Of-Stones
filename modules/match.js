    ...
    var ObjectId = require('mongodb').ObjectID;
     ...
     app.get('/match/getMatch', function(req, res){
       ...
       if(sess && sess.connectedUser){
         losDB.collection('Match').findOne({$or: [{"player1.id" : sess.connectedUser._id}, {"player2.id" : sess.connectedUser._id}]}, function(err, result){
           if(err != null || !result){
             ...
             if(!result.status){
               ...
               losDB.collection('Match').update({"_id" : new ObjectId(result._id)},{$set: {status: result.status}});
             }
             else{
               if(result.player1.board === undefined && 
                  (result.player1.deck !== undefined && result.player1.deck !== null && result.player1.deck.length > 0) && 
                  (result.player2.deck !== undefined && result.player2.deck !== null && result.player2.deck.length > 0)){
     ...
     app.get('/match/getAllMatch', function(req, res){
       losDB.collection('Match').find().toArray(function(err, result){
         if(err != null){
...
//              }
             match.player1.deck = match.player1.deck?match.player1.deck.length:0;
             match.player1.hand = match.player1.hand?match.player1.hand.length:0;
             match.player2.deck = match.player2.deck?match.player2.deck.length:0;
    ...
    var all_defined = function(deck, cb){
      ...
      if(deck && deck instanceof Array && deck.length > 0){
        ...
        for(var card of deck){
          var result = losDB.collection('Cards').findOne({key: card.key},function(err, result){
            if(err != null){
              cb(null);
            }
            else if(result != null){
          ...
          losDB.collection('Match').findOne({$or: [{"player1.id" : sess.connectedUser._id}, {"player2.id" : sess.connectedUser._id}]}, function(err, result){
           if(err != null || !result){
             ...
             if(result.status && result.status === "Deck is pending"){
               var numPlayer = (result.player1.id == sess.connectedUser._id)?1:2;
               var player = (numPlayer == 1)?result.player1:result.player2; //get the connected player info
               if(player.deck === undefined || (player.deck instanceof Array && player.deck.length === 0)){
                 if(all_different(deck)){
                   all_defined(deck, function(syncDeck){
                     if(syncDeck != null && syncDeck.length <= 20){ // deck max size is 20
                       shuffle(syncDeck);
                       ...
                       if(numPlayer == 1){
                         ...
                         losDB.collection('Match').update({"_id": new ObjectId(curMatch._id)}, {$set: { "player2.deck" : syncDeck}}, cb);
                       }
                     }
                     else{
                       tools.sendError(res, "Error during deck synchronisation");
                     }
                   });
                 }
                 else{
                   tools.sendError(res, "There is twice the same card in your deck");
                 }
               }
               else{
               ...
               tools.sendError(res, "The match status is not pending a deck");
             }
           }
         });
        }
       ...
       if(sess && sess.connectedUser){
         losDB.collection('Match').findOne({$or: [{"player1.id" : sess.connectedUser._id}, {"player2.id" : sess.connectedUser._id}]}, function(err, result){
           if(err != null || !result){
             ...
             var numPlayer = (result.player1.id == sess.connectedUser._id)?1:2;
             var player = (numPlayer == 1)?result.player1:result.player2; //get the connected player info
             var ennemyPlayer = (numPlayer == 1)?result.player2:result.player1; //get the ennemy player info
             if(player.turn === true){
              if(card){
                ...
                if(iCard != null){
                  ...
                  if(card.attack === undefined || card.attack === false){
                    if(ennemyCard){
                        var iCardEnnemy = containsCard(ennemyPlayer.board, ennemyCard);
                        if(iCardEnnemy != null){
                          var myCard = player.board[iCard];
                          myCard.attack = true;
                          var attackedEnnemyCard = ennemyPlayer.board[iCardEnnemy];
                          if(myCard.stats.attackdamage > attackedEnnemyCard.stats.armor){
                            var playerDamage = myCard.stats.attackdamage - attackedEnnemyCard.stats.armor;
                          ...
                          else if(myCard.stats.attackdamage < attackedEnnemyCard.stats.armor){
                      ...
                      tools.sendError(res, "you need to specify an ennemyCard");
                    }
                  }
                  else{
