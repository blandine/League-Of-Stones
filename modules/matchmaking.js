  ...
  init : function(app, tools, losDB){
    ...
    app.get('/matchmaking/participate', function(req, res){
      ...
      if(sess && sess.connectedUser && sess.connectedUser.email){
        losDB.collection('Matchmaking').findOne({"user.email": sess.connectedUser.email}, function(err, result){
          if(err){
    ...
    app.get('/matchmaking/unparticipate', function(req, res){
      ...
      if(sess && sess.connectedUser && sess.connectedUser.email && sess.matchmakingId){
        losDB.collection('Matchmaking').findOne({"_id" : new ObjectId(sess.matchmakingId)}, function(err, result){
          if(err != null){
    ...
    app.get('/matchmaking/getAll', function(req, res){
      var sess = req.session;
      if(sess && sess.connectedUser && sess.connectedUser.email){
         losDB.collection('Matchmaking').find({match: {$exists: false}}).toArray(function(err, result) {
           if (err) {
